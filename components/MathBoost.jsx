...
import MathBoostLanding from './MathBoostLanding';
...
const MathBoostApp = () => {
  ...
  // Usuario actual - usando useAuth hook (eliminando duplicado)
  ...
  const startGame = async () => {
    try {
      ...
      // Continuar con el juego local si falla la base de datos
      setGameMode('playing');
      setUserAnswer('');
      setShowFeedback(false);
      setSessionStartTime(Date.now());
  ...
  const checkAnswer = async (answer) => {
    ...
    if (currentSession) {
      try {
        await recordAnswer(
          {
            number1: currentProblem.num1,
            number2: currentProblem.num2,
            ...
            {Object.values(users).map((userData) => (
              ...
              >
                ...
                <div className="text-sm font-light" style={{ color: colors.textSecondary }}>
                  {(userData.totalProblemsLifetime || 0).toLocaleString()} problemas
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        ...
        <div className="flex gap-3">
          ...
          {(gameMode === 'playing' || gameMode === 'tricksPlay') && (
            <button
              onClick={() => {
                // Completar sesión en la base de datos antes de salir
                if (currentSession) {
                  completeSession().catch(error => 
                    console.error('Error al completar sesión:', error)
                  );
  ...
  // Pantalla de bienvenida interna (legacy - no usar)
  const OldWelcomeScreen = () => {
    if (!user) return null;
          ...
          <div className={`grid ${r.gridCols} ${r.gap} mb-12`}>
            <div className={`${r.cardPadding} rounded-3xl transition-all duration-500 hover:scale-105 active:scale-95 bg-white/85 backdrop-blur-xl border border-black/5 shadow-lg hover:bg-white/95 hover:backdrop-blur-2xl hover:shadow-xl hover:border-blue-500/20`}>
              ...
              <div className="mb-4">
                <div className="flex items-baseline gap-2 mb-3">
                  <span className="text-3xl font-light" style={{ color: colors.text, fontFamily: 'Georgia, serif' }}>
                    {(user?.totalProblemsThisWeek || 0).toLocaleString()}
                  </span>
                  <span className="text-lg" style={{ color: colors.textSecondary }}>
                    / {(getWeeklyProblemsGoal(user) || 0).toLocaleString()}
                  </span>
                </div>
              ...
              <p className="text-sm text-center" style={{ color: colors.textSecondary }}>
                {(getWeeklyProblemsGoal(user) || 0) - (user?.totalProblemsThisWeek || 0) > 0 
                  ? `${((getWeeklyProblemsGoal(user) || 0) - (user?.totalProblemsThisWeek || 0)).toLocaleString()} para completar meta`
          ...
          <div className={`grid ${r.gridCols} ${r.gap} mb-12`}>
            ...
            <div className={`${r.cardPadding} rounded-3xl transition-all duration-500 hover:scale-105 active:scale-95 bg-white/85 backdrop-blur-xl border border-black/5 shadow-lg hover:bg-white/95 hover:backdrop-blur-2xl hover:shadow-xl hover:border-blue-500/20`}>
              <div className="text-center">
                ...
                <div className="text-3xl font-light mb-1" style={{ color: colors.text, fontFamily: 'Georgia, serif' }}>
                  {(user?.totalProblemsLifetime || 0).toLocaleString()}
                </div>
                <div className="text-base font-light mb-3" style={{ color: colors.textSecondary }}>
                  problemas resueltos
                </div>
        ...
        <div className="max-w-4xl w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                  ...
                  <button
                    ...
                    className={`group p-12 text-center rounded-3xl transition-all duration-500 hover:scale-105 active:scale-95 no-select transform ${
                      operation === op.key 
                    ...
                    style={{
                      ...
                      color: operation === op.key ? 'white' : colors.text
                    }}
                  >
                    <div 
            ...
            <div className="space-y-12">
              <div className="flex items-center justify-between mb-16">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-2xl transition-all duration-500 hover:scale-105 active:scale-95 text-base bg-white/85 backdrop-blur-xl border border-black/5 shadow-lg hover:bg-white/95 hover:shadow-xl"
                  style={{
                    color: '#111827',
                    fontFamily: 'Inter, -apple-system, sans-serif'
                  }}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {[2, 3, 4, 5, 6, 7, 8, 9].map(table => (
                  <button
                    ...
                    className={`p-10 text-center rounded-2xl transition-all duration-500 hover:scale-105 active:scale-95 ${
                    ...
                    style={{
                      color: selectedTables.includes(table) ? 'white' : '#111827'
                    }}
                  >
                    <div 
              ...
              <div className="text-center mt-16">
                <button
                  ...
                  className="px-6 py-3.5 text-lg font-semibold rounded-2xl transition-all duration-500 hover:scale-105 active:scale-95 disabled:opacity-50 shadow-lg hover:shadow-xl"
                  style={{
                    color: '#111827',
                    fontFamily: 'Inter, -apple-system, sans-serif',
          ...
          {setupStep === 2 && (operation === 'addition' || operation === 'subtraction') && (
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                  { key: '1-20', label: '1-20', subtitle: 'rango medio' },
                  { key: '1-50', label: '1-50', subtitle: 'rango amplio' },
                  { key: '1-99', label: '1-99', subtitle: 'desafío completo' }
                ].map(range => (
                  <button
                    key={range.key}
                    onClick={() => setNumberRange(range.key)}
                    className={`p-12 text-center rounded-3xl transition-all duration-500 hover:scale-105 active:scale-95 ${
                      numberRange === range.key 
                        ? 'bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 text-white shadow-2xl' 
                        : 'bg-white/85 backdrop-blur-xl border border-black/5 shadow-lg hover:bg-white/95 hover:shadow-xl'
                    }`}
                    style={{
                      color: numberRange === range.key ? 'white' : '#111827'
                    }}
                  >
                    <div 
                      className="text-3xl font-medium mb-2 group-hover:scale-110 transition-transform duration-300" 
                      style={{ fontFamily: 'Georgia, serif' }}
                    >
                      {range.label}
                    </div>
                    <div 
                      ...
                      style={{ color: numberRange === range.key ? 'rgba(255,255,255,0.8)' : '#6B7280' }}
                    >
                      {range.subtitle}
                    </div>
                  </button>
              ...
              <div className="text-center mt-16">
                <button
                  onClick={startGame}
                  className="px-6 py-3.5 text-lg font-semibold rounded-2xl transition-all duration-500 hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl"
                  style={{
                    color: '#111827',
                    fontFamily: 'Inter, -apple-system, sans-serif',
                    background: 'linear-gradient(135deg, #DBEAFE, #EDE9FE)'
  ...
  return (
    <div style={{ fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      {loading ? (
        ...
        <>
          {gameMode === 'welcome' && <WelcomeScreen setGameMode={setGameMode} />}
